...
const LogistikModule = {
  ...
  getMonthlyData: function() {
    const stored = localStorage.getItem("DANAMULYA_LOGISTIK_MATRIX_V4");
    if (stored) {
      try {
  ...
  saveMonthlyData: function(data) {
    localStorage.setItem("DANAMULYA_LOGISTIK_MATRIX_V4", JSON.stringify(data));
  },
...
print('DIRECTLY WROTE NEW js/logistik.js SUCCESS!')
