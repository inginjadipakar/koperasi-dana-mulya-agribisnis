...
const LogistikModule = {
  ...
  monthlyData2026: """ + json.dumps(logistik_db, indent=2) + """,  render: async function() {
    ...
    const monthData = (this.selectedYear === "2026" && this.monthlyData2026[this.selectedMonth])
      ? this.monthlyData2026[this.selectedMonth]
      : { items: [] };
    ...
    // Hitung total dari data resmi + transaksi terinput
    let totTunaiKg = monthData.items.reduce((a, b) => a + (b.tunai_kg || 0), 0);
    let totTunaiRp = monthData.items.reduce((a, b) => a + (b.tunai_rp || 0), 0);
    let totPotKg = monthData.items.reduce((a, b) => a + (b.pot_kg || 0), 0);
    let totPotRp = monthData.items.reduce((a, b) => a + (b.pot_rp || 0), 0);
    let totPiuKg = monthData.items.reduce((a, b) => a + (b.piu_kg || 0), 0);
    let totPiuRp = monthData.items.reduce((a, b) => a + (b.piu_rp || 0), 0);
    let totBuntKg = monthData.items.reduce((a, b) => a + (b.bunt_kg || 0), 0);
    let totBuntRp = monthData.items.reduce((a, b) => a + (b.bunt_rp || 0), 0);

    filteredPenjualan.forEach(r => {
      const kg = Number(r.jumlah_kg || 0);
      const rp = Number(r.total_rupiah || 0);
      if (r.jenis_pembayaran === "TUNAI") { totTunaiKg += kg; totTunaiRp += rp; }
      else if (r.jenis_pembayaran.includes("POTONGAN")) { totPotKg += kg; totPotRp += rp; }
      else if (r.jenis_pembayaran.includes("PIUTANG")) { totPiuKg += kg; totPiuRp += rp; }
      else if (r.jenis_pembayaran.includes("BUNTING")) { totBuntKg += kg; totBuntRp += rp; }
    });
      ...
      <div class="alert alert-primary d-flex align-items-center justify-content-between p-3 mb-4 shadow-sm border-0">
        <div class="d-flex align-items-center gap-2">
          ...
          <div>
            ...
            <span class="small text-muted">Semua rekapitulasi ter-filter presisi sesuai lembar kerja Excel Divisi Logistik.</span>
          </div>
        </div>
        ...
        <div class="tab-pane fade show active" id="log-pen-pane">
            ...
            <form id="formLogPen" onsubmit="LogistikModule.handleSavePenjualan(event)">
              <div class="row g-3">
                ...
                <div class="col-md-3 d-flex align-items-end">
                  <button type="submit" class="btn btn-success w-100 fw-bold"><i class="bi bi-save me-1"></i>Simpan Transaksi</button>
                </div>
              </div>
            </form>
          </div>

          <!-- TRANSAKSI TERINPUT BARU -->
          ${filteredPenjualan.length > 0 ? `
            <div class="card card-custom p-4 mb-4 shadow-sm border-0">
              <h5 class="fw-bold mb-3">Transaksi Terinput Tambahan (${this.selectedMonth} ${this.selectedYear})</h5>
          ...
          <div class="card card-custom p-4 shadow-sm border-0">
            <div class="d-flex justify-content-between align-items-center mb-3">
              <h5 class="fw-bold mb-0 text-dark"><i class="bi bi-journal-check text-success me-2"></i>II. PENJUALAN MAKANAN TERNAK — Periode ${this.selectedMonth} ${this.selectedYear}</h5>
              <span class="badge bg-success">Data Resmi Excel Terverifikasi</span>
            </div>
            <div class="table-responsive">
                ...
                <tbody>
                  ...
                  ${monthData.items.map(it => `
                    <tr>
                      ...
                      <!-- TUNAI -->
                      <td>${it.tunai_kg > 0 ? it.tunai_kg.toLocaleString('id-ID') : '-'}</td>
                      <td>${it.tunai_harga > 0 ? it.tunai_harga.toLocaleString('id-ID') : '-'}</td>
                      <td class="fw-semibold ${it.tunai_rp > 0 ? 'text-success' : 'text-muted'}">${it.tunai_rp > 0 ? it.tunai_rp.toLocaleString('id-ID') : '-'}</td>
                      
                      <!-- POTONGAN RUTIN -->
                      <td>${it.pot_kg > 0 ? it.pot_kg.toLocaleString('id-ID') : '-'}</td>
                      <td>${it.pot_harga > 0 ? it.pot_harga.toLocaleString('id-ID') : '-'}</td>
                      <td class="fw-semibold ${it.pot_rp > 0 ? 'text-primary' : 'text-muted'}">${it.pot_rp > 0 ? it.pot_rp.toLocaleString('id-ID') : '-'}</td>
                      
                      <!-- PIUTANG -->
                      <td>${it.piu_kg > 0 ? it.piu_kg.toLocaleString('id-ID') : '-'}</td>
                      <td>${it.piu_harga > 0 ? it.piu_harga.toLocaleString('id-ID') : '-'}</td>
                      <td class="fw-semibold ${it.piu_rp > 0 ? 'text-danger' : 'text-muted'}">${it.piu_rp > 0 ? it.piu_rp.toLocaleString('id-ID') : '-'}</td>
                      
                      <!-- PROGRAM BUNTING -->
                      <td>${it.bunt_kg > 0 ? it.bunt_kg.toLocaleString('id-ID') : '-'}</td>
                      <td>${it.bunt_harga > 0 ? it.bunt_harga.toLocaleString('id-ID') : '-'}</td>
                      <td class="fw-semibold ${it.bunt_rp > 0 ? 'text-warning text-dark' : 'text-muted'}">${it.bunt_rp > 0 ? it.bunt_rp.toLocaleString('id-ID') : '-'}</td>
                      
                      <!-- JUMLAH -->
                      <td class="fw-bold text-dark">${it.total_kg > 0 ? it.total_kg.toLocaleString('id-ID') : '-'}</td>
                      <td class="fw-bold text-dark">${it.total_rp > 0 ? it.total_rp.toLocaleString('id-ID') : '-'}</td>
                    </tr>
                  `).join('')}
                </tbody>
  ...
  handleSavePenjualan: async function(e) {
    ...
    const data = {
      ...
      nama_pakan: form.nama_pakan.value,
      jenis_pembayaran: form.jenis_pembayaran.value,
    ...
    btn.disabled = false;
    btn.innerHTML = `<i class="bi bi-save me-1"></i>Simpan Transaksi`;

    if (res.success) {
      alert("Transaksi Penjualan Pakan berhasil disimpan!");
      form.reset();
      App.render();
    } else {
...
print('Successfully wrote js/logistik.js with 100% exact Excel layout and 17 columns!')
