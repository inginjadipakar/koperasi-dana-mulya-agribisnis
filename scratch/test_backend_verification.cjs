  ...
  getRange(startRow, startCol, numRows, numCols) {
    ...
    return {
      ...
      setValue: function(val) {
        const rIndex = startRow - 1;
        const cIndex = startCol - 1;
        if (!self.rows[rIndex]) self.rows[rIndex] = [];
        self.rows[rIndex][cIndex] = val;
      },
      getValue: function() {
        const rIndex = startRow - 1;
        const cIndex = startCol - 1;
        if (!self.rows[rIndex]) return "";
        return self.rows[rIndex][cIndex] !== undefined ? self.rows[rIndex][cIndex] : "";
      },
...
runTest(12, "Error Handling: No stack traces or raw exceptions leaked in errors", () => {
  const errRes = doPost({ postData: { contents: "INVALID JSON" } });
  const parsed = JSON.parse(errRes.text);
  const isGenericMessage = parsed.message === "Terjadi kesalahan pada server. Silakan coba lagi.";
  const noRawSyntaxError = !parsed.message.includes("SyntaxError");
  return {
    pass: parsed.success === false && parsed.code === "SERVER_ERROR" && isGenericMessage && noRawSyntaxError,
    expected: "Generic friendly error message, NO raw SyntaxError leaked",
...
runTest(16, "Depot Stok Opname Calculation: Teoretis vs Riil Selisih", () => {
  ...
  // Penjualan: 500 Liter x 2 = 1000 Liter (from Test 7)
  // Total Pengeluaran: 1000 Liter
  // Stok Teoretis: (500 + 1000) - 1000 = 500 Liter
  // Selisih: 500 - 1400 = -900 Liter
  return {
    pass: opname.success && opname.data.stok_teoretis_liter === 500 && opname.data.selisih_liter === -900,
    expected: "Teoretis: 500, Selisih: -900",
    actual: `Teoretis: ${opname.data ? opname.data.stok_teoretis_liter : null}, Selisih: ${opname.data ? opname.data.selisih_liter : null}`
  };
});
...
// 18. Password Change Hardening Test
runTest(18, "Production Password Hardening: Change Password Endpoint", () => {
  const res = changePassword(kopSessionId, "koperasi123", "Koperasi_Pass_2026!");
  const reLoginOld = loginUser("koperasi01", "koperasi123", "Test");
  const reLoginNew = loginUser("koperasi01", "Koperasi_Pass_2026!", "Test");
  
  return {
    pass: res.success && !reLoginOld.success && reLoginNew.success,
    expected: "Password changed successfully, old pass rejected, new pass accepted",
    actual: `Change:${res.code}, OldLogin:${reLoginOld.code}, NewLogin:${reLoginNew.code}`
  };
});
